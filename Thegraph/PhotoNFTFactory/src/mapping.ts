...
export function handleAddReputation(event: AddReputation): void {
  let entity = AddedReputation.load(event.transaction.from.toHex())

  if (entity == null) {
    entity = new AddedReputation(event.transaction.from.toHex())
...
export function handlePhotoNFTCreated(event: PhotoNFTCreated): void {
  let entity = AllPhotoNFT.load(event.transaction.from.toHex())

  if (entity == null) {
    entity = new AllPhotoNFT(event.transaction.from.toHex())
